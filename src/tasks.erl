...
%%% @end
%%%-------------------------------------------------------------------
c_map(List, _) when is_list(List) == false -> no_list_error;
c_map([], _)->[];
c_map(List, Predicate)->
    [F|Rest] = List,
    [Predicate(F)|c_map(Rest, Predicate)].
...
%%% @end
%%%-------------------------------------------------------------------
c_filter(List, _) when is_list(List) == false -> no_list_error;
c_filter([], _)->[];
c_filter([H|T], Predicate)->
  Result = Predicate(H),
  if
    Result =:= true -> [H|c_filter(T, Predicate)];
    true -> c_filter(T, Predicate)
  end.
